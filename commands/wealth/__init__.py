...
_locale={
    ...
    BALANCE_DESC:{EN:"Get info about your savings.",RU:"Смотреть на сколько у вас не хватает денег."},USER_CREATED:{
    ...
    TRANSFFERED:{
        EN:"You have transferred {wealth} to the user \"{user2}\"",RU:"Вы перевели {wealth} пользователю \"{user2}\"."},
    USER1_NOT_IN_DB:{
    ...
    TARGET:{EN:"target",RU:"кому"},USER:{EN:"user",RU:"пользователь"},WEALTH_T:{EN:WEALTH_NAME_EN,RU:WEALTH_NAME_RU},
    BALANCE_CHANGED:{
        EN:"Your balance has changed!\n{old_value} >>> {new_value}.",
        RU:"Ваш баланс изменился!\n{old_value} >>> {new_value}."},BALANCE_CHANGED+"0":{
...
async def balanceopacmd(interaction: discord.Interaction,user_id: str):
    await interaction.response.defer(thinking=True,ephemeral=True)
    _T.set_language(language=interaction.locale)
...
@app_commands.rename(user1_id=namedesc()user2_id=namedesc(TARGET,_locale),value=namedesc(VALUE,_locale))
async def trasferopacmd(interaction:discord.Interaction,user1_id:str,user2_id:str,value: app_commands.Range[int,1],alarm:int,text=None):
    await interaction.response.defer(thinking=True,ephemeral=True)
...
async def db_users_autocomplite(interaction: discord.Interaction,current: str):
    ...
    ac=[]
    for _ in data:
        if current in _[1] and interaction.guild.get_member(int_[0]):
            ac.append(app_commands.Choice(name=str(_[1]),value=str(_[0])))
    return ac[:25]
